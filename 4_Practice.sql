...
SELECT
    state,
    ROUND(AVG(loan_amount),1) AS loan_amount
FROM loan_data
GROUP BY state
...
SELECT
    ...
    SUM(loan_amount) AS loan_values
FROM loan_data
GROUP BY year
ORDER BY year DESC
...
show the banks that made more than 18 loans (both approved and declined).
*/
...
/*
Create a new category that will label loans as "Small", "Medium", "Large", 
